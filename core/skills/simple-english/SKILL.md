...
description: |
  ...
  word one meaning, no jargon, no AI slop. Use it on documentation, READMEs,
  runbooks, procedures, error messages, release notes, and explanations for
...
<!-- Copied from AminBlg/SimpleEnglish 2.0.2 (MIT, see LICENSE in this folder). The kit ships it slash-only: the plugin's SessionStart/Stop hooks fight the orchestrator, and Claude Code cannot disable a plugin's hooks alone. Reply rules scoped to the invoked turn by the kit. -->
...
These reply rules apply to the text this skill was invoked to produce, in the turn it was invoked. They do not persist to later replies.
...
1. Reply, in the invoked turn only: count the sentences. Over five, cut. Search for `—`, `**`, `#`, and a line that starts with `-`. Remove each one.
